...
export const spendingInsights = inngest.createFunction(
	{ name: "Generate Spending Insights", id: "generate-spending-insights", cron: "0 8 1 * *" }, // 1st of every month at 08:00
	async ({ step }) => {
		// Compute consistent month boundary to avoid race conditions
		const now = new Date();
		...
		for (const user of users) {
			...
			// Build JSON blob for the prompt
      ...
      `.trim();
			...
			try {
				...
				const htmlBody =
					...
					aiResponse.response.candidates.length > 0 &&
					aiResponse.response.candidates[0]?.content?.parts &&
					aiResponse.response.candidates[0].content.parts.length > 0
